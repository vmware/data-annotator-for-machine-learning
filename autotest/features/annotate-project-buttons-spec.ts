...
describe('annotate project ...', () => {
    ...
    it('should annotate project with less than 6 labels successfully.', async (done) => {
        ...
        if (Project_Name_Text !== '' || Project_Count_After_Filter > 0) {
            ...
            since('project info should show up and content correct').expect(annotatePage.getProjectInfo()).toEqual({ name: project_name, owner: Constant.username, source: 'text-test-data.csv', instruction: 'this is e2e test project task instruction' })
            since('progress shoud show up and content correct').expect(annotatePage.getProgress()).toEqual({ sessions: '27', annotations: '0' });

            await annotatePage.selectAnnoteLable();
            await annotatePage.waitForPageLoading();
            await browser.sleep(2000);
            since('the progress annotations should increas 1').expect(annotatePage.getProgress()).toEqual({ sessions: '27', annotations: '1' });
