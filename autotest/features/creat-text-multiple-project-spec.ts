...
describe('create new project ', () => {
    const Task_Instruction = projectCreateData.TextMultipleLabelsProject.Instruction;
    const New_Lable = projectCreateData.TextProject.Labels.split(',');
    const SET_DATA_SECTION = $('ul[role="tablist"] .nav-item:last-child');
    const PROJECT_TEXT_CLASSIFICATION = element(by.css('clr-dropdown-menu a[href="/projects/create/text"]'));
    ...
    beforeAll(() => {
        ...
        console.log('start to create new text multiple labels project : ' + New_Project_Name);
    })
    ...
    it('should create new text multiple labels project successfully...', async (done) => {
        ...
        if (Project_Name_Text !== '' && Project_Count_After_Filter > 0) {
            ...
            console.log('selectedFilename:::', projectsPage.getCellText(3), selectedFilename)
            since('the data source should same as the user uploaded file').expect(projectsPage.getCellText(3)).toBe('text-test-data.csv');
            since('the annotar should be the logged user').expect(projectsPage.getAnnotatorCellText()).toContain(Constant.username);
            since('the labels should contain the user typed lable').expect(projectsPage.getCellText(5)).toContain(New_Lable.join(','));
