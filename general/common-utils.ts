...
export class CommonUtils extends CommonPage {
    ...
    static async deleteFunction(name: string) {
        try {
            let nameText = await this.commonPage.Table_LISTS.last().element(by.css('clr-dg-cell[role="gridcell"]:nth-of-type(1)')).getText();
            console.log("nameText is " + nameText)
            if (nameText.match(name + '.*')) {
                console.log('delete project ' + nameText);
                await FunctionUtil.elementVisibilityOf(this.commonPage.DELETE_PROJECT_BTN);
                await this.commonPage.DELETE_PROJECT_BTN.click();
                await FunctionUtil.elementVisibilityOf(this.commonPage.DELETE_PROJECT_OK_BTN);
    ...
    static async deleteProjectsLoop(projectsName: string) {
        let loopBool = true;
        while (loopBool) {
            await browser.wait(ExpectedConditions.visibilityOf(this.projcetsPage.PROJECT_NAME_FILTER_BTN), this.DEFAULT_TIME_OUT);
    ...
    static async deleteMyDatasetsLoop(myDatasetsName: string) {
        let loopBool = true;
        while (loopBool) {
            await browser.wait(ExpectedConditions.visibilityOf(this.myDatasetsPage.DATASETS_NAME_FILTER_BTN), this.DEFAULT_TIME_OUT);
            await this.myDatasetsPage.filterDatasetstName(myDatasetsName);
            let filtered_MyDatasets_Count = await this.myDatasetsPage.getTableLength();
