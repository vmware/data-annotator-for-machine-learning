...
describe('Create new project ', () => {
    const Task_Instruction = projectCreateData.NerLabelsExistingProject.Instruction;
    const SET_DATA_SECTION = $('ul[role="tablist"] .nav-item:last-child');
    const PROJECT_NER_CLASSIFICATION = element(by.css('clr-dropdown-menu a[href="/projects/create/ner"]'));
    ...
    beforeAll(() => {
        ...
        console.log('start to create ner labels existing project : ' + New_Project_Name);
    })
    ...
    it('Should create ner labels existing project successfully.', async (done) => {
        ...
        await browser.wait(ExpectedConditions.visibilityOf(SET_DATA_SECTION), Constant.DEFAULT_TIME_OUT)
        await newProjectPage.setData('ner', 1, 4);
        await newProjectPage.setNewLable([projectCreateData.NerLabelsExistingProject.Labels]);
        ...
        await browser.wait(ExpectedConditions.visibilityOf($('.datagrid-host .datagrid-row:nth-child(2)')));
        await projectsPage.filterProjectName(New_Project_Name);
        let Project_Count_After_Filter = await projectsPage.getTableLength();
        let Project_Name_Text = await projectsPage.getCellText(0);
        if (Project_Name_Text !== '' && Project_Count_After_Filter > 0) {
            since('the project name should same as the user typed name').expect(projectsPage.getCellText(0)).toBe(New_Project_Name);
            since('the data source should same as the user uploaded file').expect(projectsPage.getCellText(3)).toBe(projectCreateData.NerLabelsExistingProject.Source);
            since('the annotar should be the logged user').expect(projectsPage.getAnnotatorCellText()).toContain(Constant.username);
            since('the labels should contain the user typed lable').expect(projectsPage.getCellText(5)).toContain(projectCreateData.NerLabelsExistingProject.Labels + ',' + projectCreateData.NerLabelsExistingProject.existingLabels);
            since('should have 4 actions').expect(projectsPage.getActionsCount()).toBe(5);
            done();
        }
        else {
