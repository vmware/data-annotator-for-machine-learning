...
describe('annotate project ...', () => {
    ...
    it('Should annotate image project successfully.', async (done) => {
        ...
        if (Project_Name_Text !== '' || Project_Count_After_Filter > 0) {
            ...
            since('project info should show up and content correct').expect(await annotatePage.getProjectInfo()).toEqual({ name: project_name, owner: Constant.username, source: projectCreateData.ImageProject.Source, instruction: projectCreateData.ImageProject.Instruction })
            since('progress shoud show up and content correct').expect(await annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.ImageProject.ticketSessions), annotations: '0' });


            await annotatePage.annotateImage();
            await annotatePage.waitForPageLoading();
            await browser.sleep(5000);
            since('the progress annotations should increas').expect(annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.ImageProject.ticketSessions), annotations: '1' });
            since('the history list should increase').expect(await annotatePage.getHistoryLists()).toBe(1);
