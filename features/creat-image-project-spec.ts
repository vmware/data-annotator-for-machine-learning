...
describe('Create new project ', () => {
    ...
    const PROJECT_NER_CLASSIFICATION = element(by.css('clr-dropdown-menu a[href="/projects/create/image"]'));
    ...
    it('Should create image project successfully.', async (done) => {
        ...
        await newProjectPage.setNewLable(projectCreateData.ImageProject.Labels.split(','));
        ...
        if (Project_Name_Text !== '' && Project_Count_After_Filter > 0) {
            ...
            since('the data source should same as the user uploaded file').expect(projectsPage.getCellText(3)).toBe(projectCreateData.ImageProject.Source);
            since('the annotar should be the logged user').expect(projectsPage.getAnnotatorCellText()).toContain(Constant.username);
            since('the labels should contain the user typed lable').expect(projectsPage.getCellText(5)).toContain(projectCreateData.ImageProject.Labels);
