...
describe('annotate project ...', () => {
    ...
    it('Should annotate ner labels existing project successfully.', async (done) => {
        ...
        if (Project_Name_Text !== '' || Project_Count_After_Filter > 0) {
            ...
            since('project info should show up and content correct').expect(annotatePage.getProjectInfo()).toEqual({ name: project_name, owner: Constant.username, source: projectCreateData.NerLabelsExistingProject.Source, instruction: projectCreateData.NerLabelsExistingProject.Instruction })
            since('progress shoud show up and content correct').expect(annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.NerLabelsExistingProject.ticketSessions), annotations: '0' });
            ...
            since('the progress annotations should increas 1').expect(annotations).toEqual({ sessions: String(projectCreateData.NerLabelsExistingProject.ticketSessions), annotations: '1' });
            ...
            await annotatePage.waitForPageLoading();
            await browser.sleep(2000);
            since("the progress annotations shouldn't be changed").expect(annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.NerLabelsExistingProject.ticketSessions), annotations: annotations.annotations });
            ...
            await annotatePage.waitForPageLoading();
            await browser.sleep(2000);
            since('the content should not be empty').expect(annotatePage.currentNerTicketContent()).not.toEqual('');
            since('the history list should increase 1').expect(await annotatePage.getHistoryLists()).toEqual(historyLists + 1);
