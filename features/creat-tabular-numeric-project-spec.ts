...
describe('Create new project ', () => {
    const Task_Instruction = projectCreateData.TabularNumericLabelsProject.Instruction;
    ...
    const PROJECT_TABULAR_CLASSIFICATION = element(by.css('clr-dropdown-menu a[href="/projects/create/tabular"]'));
    ...
    beforeAll(() => {
        ...
        console.log('start to create new tabular numeric labels project : ' + New_Project_Name);
    })
    ...
    it('Should create new tabular numeric labels project successfully.', async (done) => {
        ...
        if (Project_Name_Text !== '' && Project_Count_After_Filter > 0) {
            ...
            since('the data source should same as the user uploaded file').expect(projectsPage.getCellText(3)).toBe(projectCreateData.TabularNumericLabelsProject.Source);
            since('the annotar should be the logged user').expect(projectsPage.getAnnotatorCellText()).toContain(Constant.username);
            since('the labels should contain the user typed lable').expect(projectsPage.getCellText(5)).toContain(Min_Lable + '--' + Max_Lable);
