...
describe('annotate project ...', () => {
    ...
    it('Should annotate project with less than 6 labels successfully.', async (done) => {
        ...
        if (Project_Name_Text !== '' || Project_Count_After_Filter > 0) {
            ...
            since('project info should show up and content correct').expect(annotatePage.getProjectInfo()).toEqual({ name: project_name, owner: Constant.username, source: projectCreateData.TextProject.Source, instruction: projectCreateData.TextProject.Instruction })
            since('progress shoud show up and content correct').expect(annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.TextProject.ticketSessions), annotations: '0' });

            await annotatePage.selectAnnoteLable();
            await annotatePage.waitForPageLoading();
            await browser.sleep(2000);
            since('the progress annotations should increas 1').expect(annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.TextProject.ticketSessions), annotations: '1' });
