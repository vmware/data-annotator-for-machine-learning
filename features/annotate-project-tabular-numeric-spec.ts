...
describe('annotate project ...', () => {
    ...
    it('Should annotate tabular numeric labels project successfully.', async (done) => {
        ...
        if (Project_Name_Text !== '' || Project_Count_After_Filter > 0) {
            ...
            since('project info should show up and content correct').expect(annotatePage.getProjectInfo()).toEqual({ name: project_name, owner: Constant.username, source: projectCreateData.TabularNumericLabelsProject.Source, instruction: projectCreateData.TabularNumericLabelsProject.Instruction })
            since('progress shoud show up and content correct').expect(annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.TabularNumericLabelsProject.ticketSessions), annotations: '0' });

            await annotatePage.inputNumericLable();
            await annotatePage.waitForPageLoading();
            await browser.sleep(2000);
            since('the progress annotations should increas 1').expect(annotatePage.getProgress()).toEqual({ sessions: String(projectCreateData.TabularNumericLabelsProject.ticketSessions), annotations: '1' });
