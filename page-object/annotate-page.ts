...
export class AnnotatePage extends CommonPage {
    ...
    LOG_FREETEXT = element(by.css('textarea[formcontrolname="freeText"]'));
    LOG_FILTER_WORD = element(by.css('div.filterBox span.filterLabel'));
    LOG_LINES_SPAN = element.all(by.css('div p.txtRow span.rowIndex'));
    LOG_LINES_P = element.all(by.css('div p.txtRow'));
    REVIEW_TAB = element.all(by.css('li button[role=tab]'));
    LOG_FILENAME_INPUT = element(by.css('ng-select[notfoundtext="No file found"] div.ng-input input'));
    LOG_CURRENT_FILENAME = element(by.css('ng-select[notfoundtext="No file found"] span.ng-value-label'));
    LOG_FILENAME_DROPDOWN = element.all(by.css('ng-dropdown-panel div.ng-option span.ng-option-label'));
    LOG_LINES_INDEX = element(by.css('div p.txtRow0 span.logIndex0'));
    PASS_BTN = element(by.css('button.btn.btn-outline clr-icon[shape=checkbox-list]'));
    ...
    async getTotalLogLines() {
        return await this.LOG_LINES_SPAN.count();
    }


    async annotateLog(freetext) {
    ...
    async reviewLog(freetext) {
        console.log('start to review log')
        await FunctionUtil.elementVisibilityOf(this.LOG_LINES_INDEX);
        await this.LOG_LINES_INDEX.click();
    ...
    async clickReviewTab() {
        return await this.REVIEW_TAB.then(async (tabs) => {
            await FunctionUtil.elementVisibilityOf(tabs[1]);
            await tabs[1].click();
        })
    }
    ...
    async annotateImage() {
        ...
        await FunctionUtil.mouseDragMove(this.CANVAS, projectCreateData.ImageProject.firstStart, projectCreateData.ImageProject.firstEnd);
        await FunctionUtil.mouseDragMove(this.CANVAS, projectCreateData.ImageProject.secondStart, projectCreateData.ImageProject.secondEnd);
    ...
    async deleteImageRectLabelDom() {
        ...
        await FunctionUtil.mouseMoveToClick(this.CANVAS, projectCreateData.ImageProject.deletePosition);
