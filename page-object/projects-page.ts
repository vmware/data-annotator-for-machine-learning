...
export class ProjecstPage extends CommonPage {
    ...
    LATEST_DATA_TAB_FLAG = element(by.css('ul[role=tablist] li:last-child button'));
    TABLE_TOTAL_ITEMS = element(by.css('clr-dg-pagination div.pagination-description'));
    DELETE_FLAGGED_TICKET_BTN = $('.datagrid-host .datagrid-row:nth-child(2) clr-dg-cell[role="gridcell"]:last-child button:first-child');
    SILENCE_FLAGGED_TICKET_BTN = $('.datagrid-host .datagrid-row:nth-child(2) clr-dg-cell[role="gridcell"]:last-child button:last-child');
    ...
    async waitForUserChartLoading() {
        return browser.wait(ExpectedConditions.invisibilityOf(element.all(by.css(".categoryChart .spinner")).first()), Constant.DEFAULT_TIME_OUT);
    }

    async waitForCategoryChartLoading() {
        return browser.wait(ExpectedConditions.invisibilityOf(element.all(by.css(".categoryChart .spinner")).last()), Constant.DEFAULT_TIME_OUT);
    }
